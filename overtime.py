...
csvfile_name = os.getenv("HOME") + "/Dropbox/src/statistics/overtime.csv"
with open(csvfile_name, 'w') as csvfile:
    writer = csv.writer(csvfile)

    writer.writerow(["month_string", "contributors", "density", "patches"])

    while True:
        prev_date_obj = decrement_month(curr_date_obj)
        # note that the curr_date_obj is the next_month_first_day
        month_string = prev_date_obj.strftime("%Y-%m")
        ...
        print month_string, contributors, density, patches
        writer.writerow([month_string, contributors, density, patches])
